...
- **新标的建仓前先过研究闸**：`python3 scripts/data/entry_gate.py assess memory/entry-gates/{TICKER}-{date}.json`。
...
- **thesis 状态只能由 registry 改**：`memory/theses/*.json` 是唯一 baseline；改动走 `thesis_registry.py drift`，每个 improved/weakened 维度必须附上次检查之后新观察到的 evidence ID。价格波动只能改估值，动不了生意/护城河/管理层。没有 baseline 就诚实报 `unknown`。
...
- 报告长度：Mode 6 / Mode 7 目标 ≤2200 字；>3000 字 warn，>3500 字 fail；brief 无固定上限但段要齐
...
- 绕过 `openclaw cron edit` / `sync_us_cron_dst.py` 直接改 cron SQLite/旧 `jobs.json`；contract 改动必须重生成 `docs/operations/cron-schedules.md` 并跑 `scripts/system_check.py`
