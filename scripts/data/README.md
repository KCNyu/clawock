...
这不是"最全 A股 数据库"——clawock 是**美股 + 港股 + 黄金定投**的实盘组合，工具包只做实盘真正用得到的数据，且**每一条都在 kcn 服务器 IP 上实测过可达性**（业内那些"N端点"仓库通常不标这个，很多端点换个 IP 就是死的）。设计三原则:

- **无 key 优先** — 能用公开端点绝不要 API key；需 key 的（Finnhub）都有免 key fallback。
- **多源降级** — 关键路径都是 provider chain，主源挂了自动落下一个，抓空保留旧值不整片覆盖。
- **诚实可达** — 下表 `可达` 列是本机实测，不是文档宣称：✅ 稳定 · 🟡 flaky/限流 · 🔴 本机被封（保留代码，换 IP 可用）。
...
**东财统一出口 `_em_http.em_get()`** — 抄自 [simonlin1212/a-stock-data](https://github.com/simonlin1212/a-stock-data) 的 `em_get()` 防封思路(Apache-2.0)。东财对同 IP 高频请求阶梯式惩罚(先 000 空响应, 后短时 ban)，所有东财调用(`_em_symbols` / `fetch_fundamentals_em` / `fetch_fundflow_em` / `fetch_em_news`)统一走它:
...
| | 本工具包(stock-data) | 全 A股 仓库(如 a-stock-data) |
|---|---|---|
| 市场 | 美股 + 港股 + 黄金(实盘驱动) | A股 全覆盖 |
| 端点取舍 | 只做实盘用得到的 | 43 端点求全 |
| 可达性 | **每源本机实测标注** | 文档宣称, 换 IP 半数失效 |
| Key | 无 key 优先 + 免 key fallback | 部分需 key(iwencai 等) |
| 防封 | `em_get` 串行+抖动+session | `em_get` 同思路(本包已抄) |

> 若将来开 A股 实盘仓，优先只接东财 datacenter 稳的那几个(基本面/资金流/龙虎榜)，不照抄全 43 端点。
