...
def main(argv=None):
    ...
    path = decision_v2.LEDGER
    if args.list:
        for d in decision_v2.load_decisions(path):
    ...
    # Load inside the lock: a copy read before a cron's settle and written after
    # it would silently drop that settle (#1482).
    with decision_v2.ledger_lock(path):
        rows = decision_v2.load_decisions(path)
        found = False
        for d in rows:
        ...
        decision_v2.write_decisions(rows, path)
