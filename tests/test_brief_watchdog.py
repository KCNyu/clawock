...
def _no_live_scheduler(monkeypatch):
    ...
    Unstubbed that is a real read of whatever this host's runtime state DB
    happens to hold. The budget line has
    its own suite; here the scheduler is simply absent, which is the case that
    must leave every assertion below unchanged.
    """
    monkeypatch.setattr(watchdog, "brief_cron_job_state", lambda: None)
