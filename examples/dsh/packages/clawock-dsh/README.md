...
点它在中间主栏打开**全局余额面板**(再点一次或点任意会话回到对话),看全部 provider 明细(每窗口一行文字读数 + 发丝进度条;条的
填充色按用量档位走:低=绿、≥60% 黄、≥80% 红,阈值由 lowPct 派生)与手动
刷新。

挂载点是 DSH 公开的 slot:`sidebar.footer.action`(侧栏底部 Settings 旁的动作
座位)+ keyed `main`(按 id 选中的中间全局面板,宿主契约写明「不绑定 Session」),
经 `ctx.layout.selectPanel` 切换;这两样 DSH `0.1.5-rc.1` 起才齐全。更早的宿主
没有 `selectPanel`,插件自动退回会话标题行右侧 utilities 座位的毛玻璃芯片
(行为同上,面板是弹出层)。它不是 Decision Mind 的一部分:账户状态是应用级 chrome,
