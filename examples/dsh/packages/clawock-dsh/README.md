...
点开小面板看全部 provider 明细(每窗口一行文字读数 + 发丝进度条;填充是
统一的中性墨水色——就是一条普通进度条,不随用量变色,唯一例外是刷新失败
的 stale 黄)与手动刷新。它不是 Decision Mind 的一部分:账户状态是应用级 chrome,
不是交易语义。配额读数一律是**已使用 %**(kcn:「剩余」不直观):
...
- 低额红点:DeepSeek ≤¥20、MiniMax/Claude **已使用 ≥80%**(即剩余 ≤20%);
  ...
  方向翻转了;圆点仍是 provider 状态灯(绿正常/黄过期/红异常),但头条数字
  与进度条不上用量色(kcn 反馈:普通进度条就好),数字只在出错时离墨——
  金额行 low 红、stale 黄;
