...
- **`scripts/data/build_dashboard.py`**：聚合 `portfolio.json` + `memory/snapshots/` + `memory/*-plan.json` + `memory/calibration.csv` + `memory/peer-map.json` + `assets/data/risk.json` + 最近 brief context → `assets/data/dashboard.json`（v2.x schema 含 delta / today_movers / anomalies / peer_divergence / calibration / recent_plan_actions / drawdown / sector_exposure / leveraged_etf / all_time_extremes / today_ranges / realized_vs_unrealized / risk 13 个新字段；`calibration.vs_baseline.by_regime` = 主动 vs 全持有 alpha 按市场 regime 分桶，读 `lev_regime.json.regime_history`）。**brief/report postflight 自动调起**，手动也可以跑一次刷新 Pages 数据。size cap 200KB（超限先砍 recent_plans）。
