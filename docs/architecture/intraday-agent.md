...
| add-side line matches `add_side_reads` | gate | harness renders it | the model cannot rewrite a verdict it does not print |
...
| this contract | this PR |
| core packet + reference tool | planned |
| add-side line, `下一触发`, WeChat bold per channel, `🟠` warning banner | planned |
