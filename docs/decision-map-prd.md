<!-- Historical specification, split out of docs/decision-map.md on 2026-09-23.
     Not a runbook: the current behaviour is documented in docs/decision-map.md. -->

> **Historical PRD (2026-08).** Kept for comparison with what was built; read
> [`decision-map.md`](decision-map.md) for current behaviour. Known differences:
> the map now lives in the dashboard's Reflect view (#1420); its tests are
> `tests/test_decision_map.py` and `tests/decimap_board.spec.js`, not the §8 names;
> §11.5 Level 2 (writing `at_snapshot` back into the ledger) was later decided
> against — the ledger stays immutable and the read side covers the need.
> Appendix B, which pointed at private host memory files, was removed: external
> agent memory does not belong in this public repository.
...
> **KPI 条 + 一块可展开的板（源 → 信号 × 动作）+ 时间线带 + 抽屉**。
> 下文里 `decision_signal_matrix` 这个字段**已经不存在**（它最后只是一个指向
> `info_source_cards[].by_action` 的指针，板建好之后连指针也没有读者了），
> `source_kind_cards` 与 `codes` 是新的。语义没变的部分——join 的单向性、
> 年龄上限、覆盖率的读法、降级阶梯——仍然照本文。
...
### Phase 5(实为重构):收敛成一块板(已交付,#1211)

kcn:「信息密度太大太杂乱了,最好就是收敛成一张大的信息图即可。类似于大的决策树,
或者彭博终端一样。」

- `.dm-cards`(33 张等大的卡)与 `.dm-matrix`(同一批桶排成表)合成 **`.dm-board`**:
  行是 `source_kind` 可展开成它的信号,列是动作,格是 `n` + 中位收益 + 发散底色。
  覆盖率变成一条长度 = 覆盖率的条 —— 卡片版式里「不在场」和「效应弱」占一样大的面积。
- **发散色不用站内的红绿**:`#16a34a` vs `#dc2626` 在 deuteranopia 下 ΔE **5.0**
  (dataviz 校验器实测),低于 6 的地板;几百个色块上这就是「整块板对色觉障碍读者是一片灰」。
  改成蓝↔红(ΔE 27.3 protan / 34.8 normal,dark 模式 19.2 / 28.4),中点是卡片底色,
  **每格都印带符号的数字**,颜色只是重复它。时间线上的动作标记同时给了形状。
- 满色刻度取全板 |中位| 的 **90 分位**(不是最大值:一个 12 条决策的桶中位 −35%,
  按它标定会让其余每一格都没有颜色),超出截到满色,图例里写明。
  刻度**不随展开变化**,否则同一个值会因为别处展开而换颜色。
- `source_kind_cards` 是新发的**去重**汇总:一条同时 join 到 `quant.rsi14` 与
  `quant.trend` 的决策,quant 只数一次。浏览器里 sum 会多数一倍
  (实测 bar:去重 142 vs 求和 1419)。
- `codes` 把 `ticker/plan_date/action/driven_by/strategy_id/outcome` 六列改成
  「词表发一次 + 整数下标」:实测省 34KB(190KB → 156KB),这是 `signal_order`
  在上一层已经讲过的同一个道理。
- `decision_signal_matrix` 删除。
