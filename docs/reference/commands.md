...
- **GHA LLM client**：由 `clawock.automation` 持有，供 GitHub Actions 直调（绕过 OpenClaw gateway）。**Primary MiniMax M3 → optional fallback Xiaomi MiMo v2.5-pro**；不是 OpenClaw 或人工调用入口。
