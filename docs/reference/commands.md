...
> `ops/ci/generate_tool_reference.py` from the `clawock` distribution's two
> command registries (`clawock.cli.PACKAGED_UTILITIES` and the root
> `[project.scripts]`) and
...
|---|---|---|
| `clawock cron-trigger` | `clawock.automation.cron_trigger` | fires a host-triggered OpenClaw cron job; it collects nothing and decides nothing |
...
- **`clawock-influencer-scan`**：高影响力人物市场异动 → `assets/data/influencer_feed.json`。8 个源：Trump 原帖(trumpstruth.org/feed RSS, 全文) · Musk(Google News RSS 代理, X 无可靠免费 RSS) · Cathie Wood/ARK 日度调仓(arkfunds.io JSON，一手：ticker/方向/股数/占 ETF 比重) · Serenity(Substack 公开帖) · 人物报道代理(段永平/洪灏 zh-HK、Burry/Pelosi en-US)。名册+每源候选预算见 `influencer.py` 的 `_sources()`（没有 per-source cap 的话，话多的源会把 LLM 那批整个吃掉）。关键词预筛 → 单次 vendor LLM(`thinking_disabled` 结构化抽取)提相关性/stance/ticker/板块 → 代码交叉匹配持仓分三档：`held_hits` / `new_ideas` / `sector_hits`。merge-not-overwrite: 源**抓取失败**才保留旧条目（被 LLM 筛掉≠失败；Serenity 空是常态不参与保留）。LLM 只使用 `MINIMAX_API_KEY`；过滤失败时不发布未评分条目，只保留上一轮仍在窗口内的已评分条目（`retained_from_previous`，`llm_filter_status=failed_kept_previous`）。dashboard「影响力雷达」卡 + brief `▎名人异动/政策风向` 段消费。
- **GHA LLM client**：由 `clawock.automation` 持有，供 GitHub Actions 直调（绕过 OpenClaw gateway）。**MiniMax M3 (Anthropic Messages)，唯一 provider；2026-09-13 起不再有 OpenCode 回退**；不是 OpenClaw 或人工调用入口。
