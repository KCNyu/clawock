...
### 遗留事项(下一步)
1. **npm publish clawock-dsh**(需 kcn 的 npm 发布凭据,本机已就绪)。发布后 README 的 `dsh plugin --profile web add clawock-dsh` 才生效。状态与命令见本地 shared memory(`.clawhub/local-memory.md`,gitignored)
...
- **npm registry**:官方分发渠道,唯一待办(发布凭据,细节见本地 shared memory)
...
### 遗留(未完成)
1. **npm publish clawock-dsh**:workflow #581 已就绪;发布状态与凭据细节见本地 shared memory(`.clawhub/local-memory.md`,gitignored)。发布后 YELEBAI 自动收录 + README 安装命令生效
