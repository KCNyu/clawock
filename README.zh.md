...
一个真实港股 + 美股账户上的 AI 投研台,全程公开运行、公开自评;它背后的决策工作流被打包成 clawock,可装进任何外部 Agent —— 证据、反方、确定性对账,以及连接结果的改进。
...
**它首先是一份公开运行的实验记录。** 一套多 Agent 投研台在监控一个真实券商账户(港股与美股分账),每天辩论证据、提出交易建议——执行始终留在账户所有者手里。这个项目的核心产品就是这份实时记录:真实持仓、不断累积的决策历史、以及公开战绩。**亏损也在内,包括主动判断至今没跑赢买入持有。** 这不是一夜暴富的机器人,也不是跟单服务;模型只负责提议,价格、风控上限、账本、结算与评分,全部由 Python 负责。

**其次,这套决策流程被打包成了一个可安装的工具。** 它发布在 PyPI 上——`pip install clawock`——可以在你自己的账本上跑,不需要这个仓库。OpenClaw、Hermes、Claude Code、Codex 或其它外部运行时(runtime)负责模型调用、对话、记忆、规划、工具、权限和凭证;clawock 负责安装可复用的工作流(workflow):认证证据、强制反方、校验资金和汇率、连接结果,并让每个改进提案可审、可回滚。

**诚实是默认设置,不是卖点文案。** 模型不能给自己打分,战绩由 Python 独立结算并公开——包括那些不好看的结果。

### 有什么不一样

- **是工作流插件(workflow plugin),不是另一个 Agent。** 外部运行时保留自己的模型、聊天、记忆库、技能引擎与工具循环;clawock 让投资决策契约跨运行时迁移。
- **闭环不会停在答案。** 证据、反方、论点(thesis)、决策、执行与结果共用同一条血缘(lineage)。结果可以提出有边界的参数改动,却不能暗改策略。
- **真金白银,公开打分。** 一个在跑的港股 + 美股真实券商账户,公开战绩保留每一条符合条件的结果——亏损也在内,包括主动判断至今没跑赢买入持有。
- **模型不能给自己打分。** LLM 提出交易建议;Python 独立结算并计算战绩。
- **一个论点,只算一个案例(episode)。** 同一论点的重复意见只计一次。每个案例都用指定基准供应商的行情结算;缺场次(session)时按公开的补齐规则处理。
- **账本必须对得上。** 每次 push 前都检查资金守恒;现金、持仓与盈亏不平,就什么都不发布。
- **为持续运行而建。** 定时的港股与美股场次产出双语简报,并在交易日里刷新实时仪表盘。

## 快速开始:在你自己的账本上跑

**前置要求:** Python ≥ 3.11;一个能读文件、能写 `decision.json` 的外部 Agent(OpenClaw / Claude Code / Codex / 其它都行——模型调用完全留在你的运行时里)。不需要 Git,不需要这个仓库。
...
**接下来会发生什么:** `run prepare` 产出一份请求文件(含上下文指纹与工作流参数),把它交给你的 Agent;Agent 阅读上下文、写出 `decision.json`;`clawock run publish` 校验它(证据、反方、资金与汇率对账),通过后给出关联的生成回执(generation receipt)。**从安装到拿到第一张被校验过的决策单,全程不需要这个仓库,也不需要跑任何模型。**

不想先接模型?`examples/minimal-run` 在干净环境里完整跑通 `init → run prepare → run publish` 整个生命周期(它被 CI 执行,不会悄悄烂掉),可以作为无模型的冒烟(smoke)示例:

```bash
bash examples/minimal-run/run.sh
```

对于 KCNyu 兼容面,`clawock doctor`、`clawock context audit` 和 `CLAWOCK_WORKSPACE` 会检查或指向一个实际账本。它们会直说缺什么,不假装任意外来工作区都能直接运行这套 live desk。

## 怎么跑的

产品边界很简单:外部 Agent 负责读取与推理;clawock 负责可迁移的决策工作流和周围的确定性真值。

![clawock 产品架构 —— 外部运行时拥有模型、对话、记忆与工具;包提供可迁移工作流、认证上下文、确定性对账、评估和有边界改进](site/assets/product-architecture.svg)

KCNyu 部署再把这个产品边界用于一个真实组合。下面第二张是实例架构,不是可复用包架构。
...
读市场是 LLM 干的大部分活,所以整套系统最宽的一环是数据收集。仓库编录了 **8 层、40 个抓取与计算模块**,**港股与美股双语覆盖** —— 实时报价、SEC + 东财申报、资金流、财报日历、宏观(VIX / DXY / 10Y)、Reddit 与新闻情绪,以及能撬动行情的社交 feed。每份简报只取与该市场、该场次相关的子集。信息收集保持宽,决策层保持窄。
...
采集面很宽,但没有哪次运行拿全部。每个排程任务的 preflight 只组装这次能用得上的块,写进上下文文件,模型读文件而不是自己去抓。
...
| **信号** | 量化因子及其命中率复核、截面因子、同业残差、T+0 牌面 | 板块 / 同业扫描 | 板块 / 同业扫描、T+0 牌面、异动标记、盘中重跑的入场 setup 与早期趋势候选 |
| **消息与事件** | 证据图、中文公司新闻、催化日历、宏观、Reddit 与社交源 | 只对异动票的催化探针 | 只对异动票的催化探针 |
| **研究状态** | 论点登记册、研究待办队列(该复盘的财报、逾期承诺、没过闸的仓位) | 异动票的论点与红线 | 异动票的论点与红线 |
| **历史** | 回顾、决策指标、反思、数据完整性报告 | — | heartbeat 槽位状态 |
| **当日计划** | 由它写出 | 本腿今早还没成交的决策 | 本腿今早还没成交的决策 |

催化探针是里面最讲时效的一条:**只对已经异动的票**触发,一手源优先(SEC 受理时间戳、港交所公告),每条分成 interrupt / context / noise 三级,找不到就明写 `no_recent_filing`(无近期申报)——不让一个空块被读成「什么都没发生」。
...
- **多条策略,分别打分。** `core_position`、`risk_rebalance`、`intraday_t`、`event_trade`、`tactical_entry` 可以在同一只标的上并存,因为长线论点和日内交易本就可能合理地分歧。每条在自己的案例里打分。
- **归因优先。** 每条决策按其主导驱动打标签,而那个驱动的优势(edge)是从记录里*动态*测出来的 —— 逻辑里不硬编码任何命中率。

### 低频加仓战役(campaign)

加仓使用有状态的交互信号,不把均线包装成 alpha。港美分别排名:截面因子和同行残差合并为一个 `price_relative` 证据族;时点新闻通过可靠的正向 surprise,或相对同一只票自身历史突然加速的 source-weighted attention,形成另一个证据族。两族必须同时成立,负面信息或同行 laggard 证据优先阻断。进入和退出使用不同 rank 阈值,为已开启的战役提供滞回,避免一次小幅排名抖动反复开关权限。

权限、仓位和执行三层分开。warming(热身)策略每只票、每个策略版本只允许采一批 2.5% 试探性(exploration)仓位;它不等于已验证(validated),也不能用于每日重置杠杆产品。不可分割的最小交易单位只有在仍低于该市场账 3% exploration 硬上限时才能补成一股/一手,昂贵 board lot 不能伪装成小样本。只有已验证的战役才能分多批向目标靠近。技术价位只负责把已经授权的一批安排在未来五个本地交易日:最早下一场次、先判失效、跳空按开盘、港美分别使用日历和交易单位。

每个持仓都会显示 `eligible`、`waiting_timing`、`risk_blocked`、`already_at_target`、`constraint_blocked` 或 `insufficient_evidence`。所以零订单可以是正常结果,但不能再只给一个裸的 `add=0`。`clawock evaluate-add-alpha` 分别比较 setup-only、price-relative、information 和 interaction 的 T+1/T+5/T+20;当前 universe 与旧新闻快照的 replay 只标 diagnostic / survivorship-limited(幸存者偏差受限),绝不冒充已验证的 alpha。

- **证伪,不证实。** 风偏向上的行情里默认 HOLD。一个看多的故事在跨过一道证伪检查、以及一道「这是不是已经被 price in 了?」的近几日涨幅测试之前,不会触发买入。
- **regime 高于择时。** 杠杆不做择时;200 日趋势 × 波动率的拨盘给它封顶。回测的教训是:优势在*于错误 regime 里降杠杆*,不在抄顶。
...
2. **触发** —— Python 用基准供应商的逐日不复权行情、按各市场自己的交易日历评判。未完成的场次不打分;缺口直接穿过触发价的按开盘价成交 —— 绝不给它一个从未出现过的价。
3. **归组** —— 同一策略的重复判断收敛成一个*案例(episode)*,所以连喊五个早上「持有」不会凭空变出五个样本。
4. **打分并发布** —— 代码结算结果、对着一条朴素的方向基线评分、再渲染。休市场次、需要人工证据的判断、当天没成交的标的,会公开标为「不可评」—— 移出胜率分母,但保留在 coverage 覆盖计数里可见,而不是被悄悄丢掉。
...
<sub>累计案例胜率对 50% 方向命中基线 —— 衡量的是方向对了多少次,不是赚了多少。买入持有的对比是持仓(Holdings)里的影子组合(Shadow Portfolio),那是另一个问题。由 GitHub Actions 每周刷新;实时数字见<a href="https://kcnyu.github.io/clawock/#drill">持仓(Holdings)标签页</a>。</sub>
...
- **未完成场次与缺失行情。** 触发与标记来自 `memory/bars/` —— 单一基准供应商的逐日不复权行情,不是交易所直连。未完成的场次永不打分。
- **重申(reaffirmation)。** 对同一策略/动作的连续重申算一个案例。把触发价重新锚到股票现在的位置,仍是重申,不是一条新判断。
- **案例聚合。** 一个案例取它自己已结算判断的*均值*,而不是选出一条代表 —— 让第一条或最后一条代言整组,能仅凭这个选择就把主动胜率在 50% 线两侧来回甩。
- **置信度校准。** 声称的置信度只保留为审计字段。严格按日期前向(prequential)的 beta-binomial 分层模型只用更早日期,估计 action × driver × condition × regime 概率;稀疏小组向宽层先验收缩,证据或后验下界不足时不允许拿信号扩仓。
...
- **因子优势(edge)** 的 date×ticker 双向聚类 bootstrap 区间必须整体落在 50% 一侧。区间跨过 50% 意味着样本还不够,这和「因子无效」是两句不同的话——两者都不进决策,但区别会被公开写出来。
- **截面层是预注册(pre-registered)的**:只有注册时点之后记录的快照才计入激活条件,回溯结果永远不能把它打开。
...
有两条性质让它不会退化成文案。页面是**从产物生成的**,不会悄悄和产物脱节;仓库里任何引用回测的数字,都必须引用一张**仍然包含这个数字**的运行卡(run card)——失效引用指向的是真证据,但那份证据已经不再支持这句话,看起来最可信,实际是错的。这两条 CI 都会红。
...
可靠性走同一条原则。每个市场播报任务都是 **preflight(Python)→ LLM → postflight(Python)**:确定性工作全在代码里,一个 push 前的闸门拒绝发布一本对不上账的账。若风险算不出来,卡片显示 **「风险无法计算」**,绝不显示绿色的「无」。多层排程、一个兜底工作流、加上看门狗(watchdog),意味着单个 LLM 卡死不再是无声的 —— 尽管这里不承诺在任何故障下都能送达。
...
| 问题 | 入口 | 数据 / 运行契约 | 复用范围 |
|---|---|---|---|
| 分析一家美股公司 | [`us-stock-analysis`](skills/us-stock-analysis/SKILL.md) | 本地行情兜底、SEC 文件、基本面、新闻 | 可随 clawock 工作区复用 |
| 分析一家港股公司 | [`hk-stock-analysis`](skills/hk-stock-analysis/SKILL.md) | 腾讯 / 东财行情对账、港股基本面、市场环境 | 可随 clawock 工作区复用 |
...
| 压测一条供应链论点 | [`serenity-skill`](skills/serenity-skill/SKILL.md) | 当前公开证据 + 本地评分卡 | 可作为手动研究框架复用 |
| 复盘一个已披露的报告期并追踪管理层承诺 | [`earnings-review`](skills/earnings-review/SKILL.md) | 一手 filing / 港交所公告、XBRL 或东财结构化交叉验证、来源清单准出闸 | 可复用;产物落在 `memory/earnings/` |
| 判断一个新标的值不值得做深度研究 | [`entry-gate`](skills/entry-gate/SKILL.md) | 工作区行情链、instrument registry、证据来源分级、确定性硬否决 | 可复用;产物落在 `memory/entry-gates/` |

这几个入口是单向串起来的——先过建仓前研究闸,再做一手财报证据,再落到规范论点与只认证据的漂移,最后才进已有的决策、风控、结算回路。每一步都写下带版本的产物给下一步读,所以后面的环节永远没法拿文案把前面那步重新推一遍。

这些入口原生依赖工作区,不是一条命令即可独立安装的通用产品。它们要求 clawock 的脚本、数据契约和记忆 / SOP 文件;公开持仓及其运行历史只属于当前这套部署。
...
**模型。** 模型选择属于外部运行时,不属于 clawock。live OpenClaw 实例可以为
每个排程任务分别固定 primary 与 fallback;供应商凭据与路由策略放在公开
仓库之外,可以在不改工作流的情况下变化。这里不存任何供应商密钥。

**写入对账。** 仪表盘产物是一代整体发布到数据面(data plane)的派生状态;扫描旁路文件(sidecar)与其他运行时状态各有自己的生产者。规则是:隔离旁路文件写者、串行化同一主机上的仪表盘构建者,并只保留一个发布实现。

- **前端直接读扫描旁路文件。** 宏观 / 情绪 / 新闻 / 影响者 feed 在加载时逐文件抓取,所以一个 GitHub Action 只提交它自己那份不相交的旁路文件 —— 写者之间不会冲突,一份扫描在它的 commit 落地那一刻就出现在页面上,无需重建。
- **仪表盘构建者共用一把锁、一份契约。** 主机上重建在一把共享文件锁(flock)上串行;每个构建者跑同一个语义-diff 助手,所以只改时钟的重写被还原,完整 generation 一起发布到数据面。
